...
namespace {
    ...
    void obtenerVecinos(sp1::Element *e, sp1::Set_Punt_Elements &out) {
        ...
        for (int i = 0; i < elemts_p1->size(); i++) {
            if ((*elemts_p1)[i]->get_estado()) {
                out.insert((*elemts_p1)[i]);
            }
        }
        for (int i = 0; i < elemts_p2->size(); i++) {
            if ((*elemts_p2)[i]->get_estado()) {
                out.insert((*elemts_p2)[i]);
            }
        }
        for (int i = 0; i < elemts_p3->size(); i++) {
            if ((*elemts_p3)[i]->get_estado()) {
    ...
    void obtenerListaVecinos(sp1::Set_Punt_Elements &listaVecinos) {
        sp1::Set_Punt_Elements temp;
        for (sp1::Element* v : listaVecinos) {
            temp.insert(v);
        }
...
namespace sp1 {
    ...
    void cambiarConductividadElementos(int num_elemento, int profundidad, float permeabilidad1, float permeabilidad2,
        ...
        sp1::Set_Punt_Elements listaVecinos;
        listaVecinos.insert(e);
        ...
        while (profundidad > 0) {
            ...
            for (Element* v : listaVecinos) {
                v->set_permeabilidad(nuevaPermeabilidad);
            }

            obtenerListaVecinos(listaVecinos);
